...
function showresults(m, drawplots, hoursperperiod, sets, params, vars, constraints)
	...
	if drawplots
		...
		countryplot(:North)
		countryplot(:Northeast)
		countryplot(:East)
		countryplot(:SouthCentral)
		countryplot(:Southwest)
		countryplot(:Northwest)
		
		if true	# plot hydro storage & shadow prices
			display(plot(existingstoragelevel[:,:hydro,:Southwest], size=(1850,950), tickfont=16, legendfont=16))

			plot(elec[:,:hydro,:Southwest]/hoursperperiod/1000, size=(1850,950), tickfont=16, legendfont=16)
